...
- (void)addRightButton2NavWithTwoTitle:(NSString *)title{
    self.navigationItem.rightBarButtonItems = @[[self titleButtonWithTwo:title left:NO enabled:YES]];
}
...
- (void)addTwoRightButton2NavWithTitleName:(NSString *)titleName imageName:(NSString *)imageName2 actionL:(SEL)action actionR:(SEL)actionRight{
    self.navigationItem.rightBarButtonItems = @[[self newTitleButtonWithImageName:titleName left:NO width:30 action:action],[self titleButtonWithImageName:imageName2 left:NO width:30 action:actionRight]];
}
- (void)addBackButton{
...
//    [button setTintColor:[UIColor whiteColor]];
    [imageView setImage:[UIImage imageNamed:@"arrow_w"]];
...
- (UIBarButtonItem *)titleButton:(NSString *)title left:(BOOL)flag enabled:(BOOL)enabled{
    ...
    [button.titleLabel setFont:UIFontOfSize(15)];
    [button setEnabled:enabled];
    return [[UIBarButtonItem alloc] initWithCustomView:button];
}
//右上角两个字
- (UIBarButtonItem *)titleButtonWithTwo:(NSString *)title left:(BOOL)flag enabled:(BOOL)enabled{
    UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
    [button setFrame:CGRectMake(0, 20, kNavButtonWidth - 20, kNavButtonHeight)];
    if (flag) {
        [button setTag:555];
        [button addTarget:self action:@selector(leftBarButtonPressed:) forControlEvents:UIControlEventTouchUpInside];
    }else{
...
- (UIBarButtonItem *)newTitleButtonWithImageName:(NSString *)imageName left:(BOOL)flag width:(CGFloat)width action:(SEL)selector{
    UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
    [button setFrame:CGRectMake(0, 0, width, kNavButtonHeight)];
    if (flag) {
        [button addTarget:self action:selector forControlEvents:UIControlEventTouchUpInside];
